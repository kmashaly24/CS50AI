...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people.keys():
        ...
        if mother is None or father is None:
            ...
            mother_prob = gene_passed(mother)
            father_prob = gene_passed(father)

            if person in two_genes:
                gene_prob = mother_prob * father_prob
                if person in have_trait:
                    prob *= gene_prob * PROBS["trait"][2][True]
                else:
                    prob *= gene_prob * PROBS["trait"][2][False]
            elif person in one_gene:
                gene_prob = (mother_prob * (1 - father_prob) +
                             (1 - mother_prob) * father_prob)
                if person in have_trait:
                    prob *= gene_prob * PROBS["trait"][1][True]
                else:
                    prob *= gene_prob * PROBS["trait"][1][False]
            else:
                gene_prob = (1 - mother_prob) * (1 - father_prob)
                if person in have_trait:
                    prob *= gene_prob * PROBS["trait"][0][True]
                else:
                    prob *= gene_prob * PROBS["trait"][0][False]
    return prob
